  ...
  return Swal.fire({
    html: `
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; padding: 8px 0;">
        ...
        ">Your food is being prepared and will arrive soon</p>
        ...
        ">
          ...
          <span style="font-size: 13px; font-weight: 600; color: #c2410c;">
...
const Checkout = () => {
  ...
  const { cgst, sgst, deliveryFee, platformFee, grandTotal } = useMemo(() => {
    if (cart.length === 0) return { cgst: 0, sgst: 0, deliveryFee: 0, platformFee: 0, grandTotal: 0 };
    const cgst        = totalPrice * 0.025;
    const sgst        = totalPrice * 0.025;
    const deliveryFee = 40;
    const platformFee = 7;
    const grandTotal  = totalPrice + cgst + sgst + deliveryFee + platformFee;
    return { cgst, sgst, deliveryFee, platformFee, grandTotal };
  }, [cart, totalPrice]);
  ...
  useEffect(() => {
    ...
    const summary = {
      ...
      totalPrice, cgst, sgst, deliveryFee, platformFee, grandTotal,
    };
  ...
  const summary = useMemo(() =>
    cart.length > 0
      ? { cart, totalPrice, cgst, sgst, deliveryFee, platformFee, grandTotal }
      : savedSummary || { cart: [], totalPrice: 0, cgst: 0, sgst: 0, deliveryFee: 0, platformFee: 0, grandTotal: 0 },
  [cart, totalPrice, cgst, sgst, deliveryFee, platformFee, grandTotal, savedSummary]);
  ...
  const saveOrderToHistory = useCallback((values) => {
    const newOrder = {
  ...
  const handleSubmit = useCallback((values, { setSubmitting }) => {
    ...
    setSubmitting(false);
  }, [user, summary, paymentMethod, saveOrderToHistory, clearCart, navigate]);
    ...
    >
              ...
              return (
                <>
                    ...
                    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
                      ...
                      <div className="space-y-2">
                        {savedAddresses.map((addr, idx) => (
                          <div
                            key={idx}
                            onClick={() => handleSelectAddress(addr, idx, setFieldValue)}
                    ...
                    <Form className="flex flex-col gap-4">
                      ...
                      <div>
                        ...
                        <div className="relative">
                          ...
                          <Field name="phone" placeholder="10-digit mobile number"
                        ...
                        <ErrorMessage name="phone" component="p" className="text-red-400 text-xs mt-1" />
                      </div>
                      <div>
                        <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5 block">Delivery Address</label>
                        <Field as="textarea" name="address" rows={3}
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          ...
                          <Field name="city" placeholder="City"
                            className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400 focus:border-transparent transition" />
                          <ErrorMessage name="city" component="p" className="text-red-400 text-xs mt-1" />
                        </div>
                        <div>
                          <label className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5 block">Pincode</label>
                          <Field name="pincode" placeholder="6-digit pincode"
                            className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400 focus:border-transparent transition" />
                          <ErrorMessage name="pincode" component="p" className="text-red-400 text-xs mt-1" />
                        </div>
                      </div>
                    </Form>
                  </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                      {[
                        ...
                        >
                          ...
                          {paymentMethod === method.id && (
                            <p className="text-[10px] text-orange-500 mt-0.5">✓ Selected</p>
                          )}
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="flex justify-center gap-6 text-xs text-gray-400">
                    {["🔒 100% Secure", "🚀 Fast Delivery", "✅ Fresh & Hot"].map((t) => (
                      <span key={t}>{t}</span>
                    ))}
                  </div>
                </>
          ...
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 lg:sticky lg:top-6">
            ...
            <div className="bg-orange-50 rounded-xl px-4 py-2.5 flex items-center gap-2 mb-4">
              ...
              <p className="text-xs text-orange-700 font-medium">
                Estimated delivery: <strong>30–40 mins</strong>
              </p>
            </div>
            ...
            <div className="border-t border-dashed border-gray-200 pt-4 space-y-2 text-sm text-gray-600">
              <div className="flex justify-between">
                ...
                <span className="font-semibold text-gray-800">₹{summary.totalPrice}</span>
              </div>
      ...
      <style>{`
        ...
        .checkout-steps .ant-steps-item-wait .ant-steps-item-icon {
          background-color: rgba(255,255,255,0.2) !important; border-color: rgba(255,255,255,0.4) !important;
        }
        ...
        .checkout-steps .ant-steps-item-tail { background-color: rgba(255,255,255,0.3) !important; }
