...
const Cart = () => {
  ...
  const handleCheckout = useCallback(() => {
    ...
    if (cart.length === 0) {
      showAlert({ title: "Cart is Empty 😢", text: "Add items first", icon: "warning" });
      return;
    }

    // Mark promo as used so it doesn't auto-fill on next order
    sessionStorage.setItem("promoUsed", "true");

    // ── KEY FIX: pass full promo details to Checkout so it can
    // apply the discount and save it in order history / invoice
    navigate("/checkout", {
      state: {
        promoCode:     promoInput,
        promoDiscount: promoDiscount,
        promoSaving:   discount,      // total saving amount shown in summary
        grandTotal:    grandTotal,
      },
    });
  }, [user, cart, navigate, promoInput, promoDiscount, discount, grandTotal]);
  ...
  const handleRemove = useCallback((id) => {
    ...
    }).then((res) => { if (res.isConfirmed) removeFromCart(id); });
  }, [removeFromCart]);
  ...
  const handleContinueShopping = useCallback(() => navigate("/menu"), [navigate]);
    ...
    >
      ...
      <div className="bg-gradient-to-r from-orange-500 to-orange-400 py-8 px-4">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          <div>
            <h1 className="text-2xl md:text-3xl font-extrabold text-white">Your Cart 🛒</h1>
              ...
              {cart.map((item) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <h2 className="font-bold text-gray-800 text-base truncate">{item.name}</h2>
                    <p className="text-xs text-gray-400 mt-0.5">Fresh ingredients · Made to order</p>
                    <div className="flex items-center justify-between mt-3 flex-wrap gap-2">
                      ...
                      <div className="text-right">
                        <p className="font-extrabold text-gray-800 text-base">₹{item.price * item.quantity}</p>
                ...
                {promoDiscount > 0 ? (
                  <div className="flex items-center justify-between bg-green-50 border border-green-200 rounded-xl px-4 py-2.5">
                    <div>
                      ...
                      <p className="text-green-500 text-xs mt-0.5">✓ You save ₹{promoDiscount} extra</p>
                    </div>
                    <button
                  ...
                  <>
                    ...
                    <p className="text-gray-400 text-xs mt-2">Example: HUNGRY50 for ₹50 off</p>
                  </>
                )}
              </div>
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 sticky top-20">
                ...
                <div className="space-y-3 text-sm">
                  ...
                  <div className="flex justify-between text-green-600">
                    <span>Discount {promoDiscount > 0 && `+ Promo (${promoInput})`}</span>
                ...
                <p className="text-[10px] text-gray-400 text-center mt-2">Inclusive of all taxes</p>
