...
const previewInvoice = (order) => {
  ...
  const promoSaving = Number(pricing.promoSaving || 0);
  ...
  const statusColors = { Confirmed:"#16a34a", Preparing:"#d97706", "On the way":"#2563eb", Delivered:"#6b7280", Cancelled:"#dc2626" };
  const sc        = statusColors[order.orderStatus] || "#16a34a";
  const paid      = order.payment?.status || "";
  const paidColor = paid === "Paid" ? "#16a34a" : "#d97706";

  // ── KEY FIX: promo row in totals — only shown if promo was applied
  const promoRow = promoSaving > 0 ? `
    <div class="totals-row" style="color:#16a34a;">
      <span>Promo (${promoCode})</span>
      <span style="font-weight:700;">&#8722; &#8377;${promoSaving.toFixed(0)}</span>
    </div>` : "";
...
.inv-meta .label{font-size:10px;opacity:.75;text-transform:uppercase;letter-spacing:.5px}
...
.info-box .lbl{font-size:10px;font-weight:700;color:#9ca3af;text-transform:uppercase;letter-spacing:.6px;margin-bottom:6px}
...
<body>
  ...
  <div class="page">
    ...
    <table>
      <thead><tr>
        <th style="width:36px">#</th><th style="text-align:left">Item</th>
...
const generateInvoice = (order) => {
  ...
  font("bold", 22); rgb(255, 255, 255);
  doc.text("HungryHub", margin, 15);
  font("normal", 8); doc.text("Fresh Food, Fast Delivery", margin, 21);
  font("bold", 10); doc.text("TAX INVOICE", W - margin, 13, { align: "right" });
  font("normal", 8);
  ...
  // Billed To / Payment
  doc.setFillColor(249, 250, 251); doc.rect(margin, y, W - 2 * margin, 28, "F");
  font("bold", 8); rgb(107, 114, 128);
  doc.text("BILLED TO", margin + 4, y + 6);
  font("bold", 9); rgb(31, 41, 55);
  doc.text(order.customer?.name || "", margin + 4, y + 12);
  font("normal", 8); rgb(107, 114, 128);
  doc.text(`${order.customer?.address || ""}, ${order.customer?.city || ""} - ${order.customer?.pincode || ""}`, margin + 4, y + 18, { maxWidth: 80 });
  doc.text(`Phone: ${order.customer?.phone || ""}`, margin + 4, y + 24);
  const cx = W / 2 + 4;
  font("bold", 8); rgb(107, 114, 128); doc.text("PAYMENT METHOD", cx, y + 6);
  font("normal", 9); rgb(31, 41, 55); doc.text(order.payment?.methodLabel || "", cx, y + 12);
  const paid = order.payment?.status || "";
  font("normal", 8);
  ...
  // Items table header
  doc.setFillColor(249, 115, 22); doc.rect(margin, y, W - 2 * margin, 8, "F");
  font("bold", 8); rgb(255, 255, 255);
  ...
  pricingRows.forEach(([label, val]) => {
    font("normal", 8.5); rgb(107, 114, 128);
  ...
  // ── KEY FIX: promo row in PDF — only if promo was applied
  if (promoSaving > 0) {
    font("normal", 8.5); rgb(22, 163, 74);
    doc.text(`Promo (${promoCode})`, W - margin - 42, y);
    doc.text(`- Rs.${promoSaving.toFixed(0)}`, W - margin - 2, y, { align: "right" });
    y += 6;
  }

  y += 1; line(y, [249, 115, 22]); y += 6;

  // Grand total
  doc.setFillColor(255, 247, 237); doc.rect(W - margin - 70, y - 4, 70, 10, "F");
  font("bold", 10); rgb(249, 115, 22);
  ...
  // Status badge
  const sc = ({ Confirmed:[22,163,74], Preparing:[161,98,7], "On the way":[37,99,235], Delivered:[107,114,128], Cancelled:[220,38,38] })[order.orderStatus] || [22,163,74];
  doc.setFillColor(...sc); doc.roundedRect(margin, y, 40, 8, 2, 2, "F");
  font("bold", 8); rgb(255, 255, 255);
  doc.text(order.orderStatus || "Confirmed", margin + 20, y + 5.2, { align: "center" });
  y += 16; line(y); y += 6;

  // Footer
  font("normal", 7.5); rgb(156, 163, 175);
...
const OrderCard = ({ order }) => {
  ...
  return (
    <div className="border border-gray-200 rounded-2xl overflow-hidden hover:border-orange-200 transition-all">
      <div className="flex items-center justify-between gap-3 p-4 cursor-pointer select-none bg-white hover:bg-gray-50 transition" onClick={() => setOpen((o) => !o)}>
        ...
        <div className="flex items-center gap-2 shrink-0">
          <span className={`text-xs font-bold px-2.5 py-1 rounded-full border ${statusClass}`}>{order.orderStatus}</span>
          <span className="font-extrabold text-orange-500 text-sm">₹{Number(order.pricing?.grandTotal || 0).toFixed(0)}</span>
          <span className="text-gray-400 text-sm" style={{ transform: open ? "rotate(180deg)" : "rotate(0deg)", display: "inline-block", transition: "transform 0.2s" }}>▾</span>
        </div>
      </div>
      ...
      {open && (
        <div className="border-t border-gray-100 bg-gray-50 p-4 space-y-4">
          <div className="flex justify-end gap-2">
            <button onClick={() => previewInvoice(order)} className="flex items-center gap-2 bg-white hover:bg-orange-50 text-orange-500 border border-orange-300 text-xs font-bold px-4 py-2 rounded-xl transition">
              <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path strokeLinecap="round" strokeLinejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.477 0 8.268 2.943 9.542 7-1.274 4.057-5.065 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
              View Invoice
            </button>
            <button onClick={() => generateInvoice(order)} className="flex items-center gap-2 bg-orange-500 hover:bg-orange-600 text-white text-xs font-bold px-4 py-2 rounded-xl transition shadow-sm shadow-orange-200">
              <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M4 16v2a2 2 0 002 2h12a2 2 0 002-2v-2M7 10l5 5 5-5M12 15V3"/></svg>
              Download Invoice
            </button>
          </div>
            ...
            <div className="space-y-2">
              {(order.items || []).map((item, i) => (
                <div key={i} className="flex items-center gap-3 bg-white rounded-xl p-2.5 border border-gray-100">
                  {item.image ? (
                    <img src={item.image} alt={item.name} className="w-12 h-12 rounded-lg object-cover shrink-0" onError={(e) => { e.target.style.display = "none"; }} />
                  ) : (
          ...
          <div>
            ...
            <div className="bg-white rounded-xl border border-gray-100 p-3 space-y-1.5 text-sm">
              {[
                ...
                ["CGST (2.5%)", `₹${Number(order.pricing?.cgst||0).toFixed(0)}`],
                ["SGST (2.5%)", `₹${Number(order.pricing?.sgst||0).toFixed(0)}`],
                ["Delivery Fee", `₹${order.pricing?.deliveryFee}`],
                ["Platform Fee", `₹${order.pricing?.platformFee}`],
              ].map(([l, v]) => (
                <div key={l} className="flex justify-between text-gray-500"><span>{l}</span><span>{v}</span></div>
              ))}
              {/* ── KEY FIX: promo row in order history UI */}
              {Number(order.pricing?.promoSaving || 0) > 0 && (
                <div className="flex justify-between text-green-600 font-semibold">
                  <span>🏷️ Promo ({order.pricing?.promoCode})</span>
                  <span>− ₹{Number(order.pricing?.promoSaving).toFixed(0)}</span>
                </div>
              )}
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="bg-white rounded-xl border border-gray-100 p-3">
              ...
              <p className="text-xs text-gray-400 mt-0.5 leading-relaxed">{order.customer?.address}, {order.customer?.city} – {order.customer?.pincode}</p>
            ...
            <div className="bg-white rounded-xl border border-gray-100 p-3">
              ...
              <p className={`text-xs mt-1 font-medium ${order.payment?.status === "Paid" ? "text-green-500" : "text-yellow-500"}`}>{order.payment?.status}</p>
            </div>
          </div>
        </div>
      )}
...
const Profile = () => {
  ...
  const originalAddrRef = useRef(null);

  useEffect(() => {
    if (!user) {
      showAlert({ title: "Login Required 🔐", icon: "warning" }).then(() => navigate("/login"));
      return;
    }
    const email = (user.email || user).toLowerCase();
    const saved = JSON.parse(localStorage.getItem(profileKey(email)));
    setProfile({ name: saved?.name || user.name || "Guest User", email: saved?.email || email, phone: saved?.phone || "" });
    setSavedAddresses(JSON.parse(localStorage.getItem(addressKey(email))) || []);
    setOrders(JSON.parse(localStorage.getItem(orderHistoryKey(email))) || []);
  }, [user, navigate]);

  useEffect(() => { if (isEditing) nameRef.current?.focus(); }, [isEditing]);

  const initial = useMemo(() => (profile.name || "G").charAt(0).toUpperCase(), [profile.name]);
  ...
  const handleSave = useCallback(() => {
    if (!profile.name || !profile.email) { showAlert({ title: "Fill all fields ⚠️", icon: "warning" }); return; }
    localStorage.setItem(profileKey(profile.email.toLowerCase()), JSON.stringify(profile));
  ...
  const handleLogout = useCallback(() => {
    logout();
    showAlert({ title: "Logged Out 👋", icon: "success" }).then(() => navigate("/login"));
  }, [logout, navigate]);

  const handleEditAddress = useCallback((idx) => {
    setEditingAddrIdx(idx);
    setAddrForm({ ...savedAddresses[idx] });
    originalAddrRef.current = { ...savedAddresses[idx] };
    setAddrErrors({});
  }, [savedAddresses]);
  ...
  const handleAddrSave = useCallback(() => {
    ...
    setEditingAddrIdx(null); setAddrForm(EMPTY_ADDR); originalAddrRef.current = null;
    showAlert({ title: "Address Updated ✅", icon: "success" });
  }, [addrForm, editingAddrIdx, user, validateAddr]);

  const handleAddrCancel = useCallback(() => {
    const original   = originalAddrRef.current;
    const hasChanges = original && Object.keys(addrForm).some((key) => addrForm[key] !== original[key]);
    if (!hasChanges) {
      setEditingAddrIdx(null); setAddrForm(EMPTY_ADDR); setAddrErrors({}); originalAddrRef.current = null;
      return;
    }
    Swal.fire({
      title: "Discard changes?", text: "Your unsaved edits will be lost.", icon: "warning",
      showCancelButton: true, confirmButtonColor: "#ef4444", cancelButtonColor: "#6b7280",
      confirmButtonText: "Yes, discard", cancelButtonText: "Keep editing",
    }).then((res) => {
      if (res.isConfirmed) { setEditingAddrIdx(null); setAddrForm(EMPTY_ADDR); setAddrErrors({}); originalAddrRef.current = null; }
    });
  }, [addrForm]);
  ...
  const handleDeleteAddress = useCallback((index) => {
    if (!user) return;
    Swal.fire({
      title: "Delete address?", text: "This address will be permanently removed.", icon: "warning",
      showCancelButton: true, confirmButtonColor: "#ef4444", cancelButtonColor: "#6b7280",
      ...
      if (editingAddrIdx === index) { setEditingAddrIdx(null); setAddrForm(EMPTY_ADDR); originalAddrRef.current = null; }
    });
  }, [user, editingAddrIdx]);

  const inputClass = (editing) =>
    `w-full px-4 py-3 border rounded-xl text-sm transition ${editing ? "border-orange-300 focus:outline-none focus:ring-2 focus:ring-orange-400 bg-white" : "border-gray-200 bg-gray-50 text-gray-500 cursor-not-allowed"}`;
  ...
  const AddressEditForm = () => (
    <div className="mt-4 border-2 border-orange-400 rounded-2xl p-4 sm:p-5 bg-orange-50 space-y-3">
      ...
      <div className="flex gap-2">
        {ADDRESS_TYPES.map((t) => (
          <button key={t} type="button" onClick={() => setAddrForm((p) => ({ ...p, type: t }))}
            className={`flex-1 py-1.5 rounded-lg text-xs font-bold border-2 transition-all ${addrForm.type === t ? "border-orange-500 bg-orange-500 text-white" : "border-gray-200 bg-white text-gray-500"}`}>
            {t === "Home" ? "🏠" : t === "Work" ? "💼" : "📍"} {t}
          </button>
        ))}
      </div>
      {[
        ...
        <div key={f.name}>
          <input name={f.name} value={addrForm[f.name]} onChange={handleAddrChange} placeholder={f.placeholder}
          ...
          {addrErrors[f.name] && <p className="text-red-400 text-xs mt-1">{addrErrors[f.name]}</p>}
        </div>
      ))}
      <div>
        <textarea name="address" value={addrForm.address} onChange={handleAddrChange}
      ...
      <div className="grid grid-cols-2 gap-3">
        {[{ name: "city", placeholder: "City" }, { name: "zipCode", placeholder: "6-digit Pincode" }].map((f) => (
          <div key={f.name}>
            <input name={f.name} value={addrForm[f.name]} onChange={handleAddrChange} placeholder={f.placeholder}
              className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400 bg-white" />
            {addrErrors[f.name] && <p className="text-red-400 text-xs mt-1">{addrErrors[f.name]}</p>}
      ...
      <div className="flex gap-2 pt-1">
        <button onClick={handleAddrSave} className="flex-1 bg-orange-500 hover:bg-orange-600 active:bg-orange-700 text-white py-2.5 rounded-xl text-sm font-bold transition">✅ Update Address</button>
        <button onClick={handleAddrCancel} className="flex-1 bg-gray-100 hover:bg-gray-200 text-gray-600 py-2.5 rounded-xl text-sm font-semibold transition">Cancel</button>
      </div>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      <div className="sm:hidden sticky top-0 z-20 bg-white border-b border-gray-100 shadow-sm">
        <div className="flex">
          {[{ key:"profile",icon:"👤",label:"Profile"},{ key:"address",icon:"📍",label:"Addresses"},{ key:"orders",icon:"📦",label:"Orders"}].map((tab) => (
            <button key={tab.key} onClick={() => setActiveTab(tab.key)}
              className={`flex-1 flex items-center justify-center gap-1.5 py-3.5 text-sm font-semibold border-b-2 transition-colors ${activeTab===tab.key?"border-orange-500 text-orange-500":"border-transparent text-gray-500"}`}>
      ...
      <div className="max-w-5xl mx-auto px-4 py-6 sm:py-10">
        <div className="sm:hidden bg-white rounded-2xl shadow-sm p-5 mb-4 flex items-center gap-4">
          <div className="w-14 h-14 shrink-0 rounded-full bg-orange-500 text-white flex items-center justify-center text-2xl font-bold">{initial}</div>
          <div className="min-w-0">
          ...
          <button onClick={handleLogout} className="ml-auto shrink-0 text-xs font-semibold text-red-500 border border-red-200 px-3 py-1.5 rounded-lg hover:bg-red-50 transition">Logout</button>
        </div>

        <div className="grid md:grid-cols-3 gap-6">
          <div className="hidden sm:flex bg-white rounded-2xl shadow-sm p-6 flex-col items-center h-fit">
            <div className="w-24 h-24 rounded-full bg-orange-500 text-white flex items-center justify-center text-3xl font-bold">{initial}</div>
            <h2 className="text-xl font-bold mt-4 text-gray-800 text-center w-full">{profile.name}</h2>
            <p className="text-gray-500 text-sm mt-1 text-center break-all w-full">{profile.email}</p>
            <div className="mt-6 w-full space-y-2">
              {[{key:"profile",icon:"👤",label:"Profile"},{key:"address",icon:"📍",label:"Saved Addresses"},{key:"orders",icon:"📦",label:"Order History"}].map((tab) => (
                <button key={tab.key} onClick={() => setActiveTab(tab.key)}
                  className={`w-full text-left px-4 py-2.5 rounded-xl font-medium transition text-sm ${activeTab===tab.key?"bg-orange-500 text-white":"text-gray-600 hover:bg-gray-100"}`}>
                  {tab.icon} {tab.label}
                </button>
              ))}
              <button onClick={handleLogout} className="w-full text-left px-4 py-2.5 rounded-xl font-medium text-red-500 hover:bg-red-50 transition text-sm">🚪 Logout</button>
            </div>
          </div>
          ...
          <div className="md:col-span-2 bg-white rounded-2xl shadow-sm p-5 sm:p-6">
            ...
            {activeTab === "profile" && (
              <>
                <div className="flex items-center justify-between mb-5 sm:mb-6 gap-3">
                  <h2 className="text-base sm:text-xl font-bold text-gray-800">Personal Information</h2>
                  {!isEditing ? (
                    <button onClick={() => setIsEditing(true)} className="shrink-0 bg-orange-500 hover:bg-orange-600 text-white px-4 sm:px-5 py-2 rounded-lg text-sm font-semibold transition">Edit</button>
                  ) : (
                    <div className="flex gap-2 shrink-0">
                      <button onClick={() => setIsEditing(false)} className="bg-gray-100 hover:bg-gray-200 text-gray-600 px-3 sm:px-4 py-2 rounded-lg text-sm font-semibold transition">Cancel</button>
                      <button onClick={handleSave} className="bg-green-500 hover:bg-green-600 text-white px-4 sm:px-5 py-2 rounded-lg text-sm font-semibold transition">Save</button>
                    </div>
                  )}
                </div>
                <div className="space-y-4">
                  {[{label:"Full Name",name:"name",placeholder:"Full Name"},{label:"Email Address",name:"email",placeholder:"Email"},{label:"Phone Number",name:"phone",placeholder:"Phone Number"}].map((f) => (
                    <div key={f.name}>
                      <label className="block text-xs sm:text-sm font-medium text-gray-600 mb-1">{f.label}</label>
                      <input ref={f.name==="name"?nameRef:undefined} name={f.name} value={profile[f.name]} onChange={handleChange} disabled={!isEditing} placeholder={f.placeholder} className={inputClass(isEditing)} />
                    </div>
                  ))}
                </div>
              </>
            ...
            {activeTab === "orders" && (
              <>
                <div className="flex items-center justify-between mb-5 sm:mb-6 gap-3">
                  <h2 className="text-base sm:text-xl font-bold text-gray-800">Order History</h2>
                ...
                {orders.length === 0 ? (
                  <div className="text-center py-10 sm:py-12">
                    ...
                    <p className="text-gray-400 text-xs sm:text-sm mt-1">Your order history will appear here</p>
                    <button onClick={() => navigate("/menu")} className="mt-4 bg-orange-500 hover:bg-orange-600 text-white px-5 py-2 rounded-lg text-sm font-semibold transition">Order Now</button>
                  </div>
                ) : (
                  <div className="space-y-4">
                    {orders.map((order, idx) => <OrderCard key={order.orderId || idx} order={order} />)}
                  </div>
                )}
              </>
            )}
            ...
            {activeTab === "address" && (
              <>
                <div className="flex items-center justify-between mb-5 sm:mb-6 gap-3">
                  <h2 className="text-base sm:text-xl font-bold text-gray-800">Saved Addresses</h2>
                  <button onClick={() => navigate("/deliveryaddress")} className="shrink-0 bg-orange-500 hover:bg-orange-600 text-white px-4 py-2 rounded-lg text-sm font-semibold transition">+ Add New</button>
                </div>
                {savedAddresses.length === 0 ? (
                  <div className="text-center py-10 sm:py-12">
                    <p className="text-4xl mb-3">📍</p>
                    <p className="text-gray-500 font-medium text-sm sm:text-base">No address saved yet</p>
                    <p className="text-gray-400 text-xs sm:text-sm mt-1">Add a delivery address to speed up checkout</p>
                    <button onClick={() => navigate("/deliveryaddress")} className="mt-4 bg-orange-500 hover:bg-orange-600 text-white px-5 py-2 rounded-lg text-sm font-semibold transition">+ Add Address</button>
                  </div>
                ) : (
                  <div className="space-y-3">
                    {savedAddresses.map((addr, idx) => (
                      <div key={idx}>
                        <div className={`border-2 rounded-2xl p-4 transition-all ${editingAddrIdx===idx?"border-orange-400 bg-orange-50/30":"border-gray-200 hover:border-orange-200 bg-white"}`}>
                          <div className="flex items-start justify-between gap-2">
                            <span className={`inline-flex items-center gap-1 text-xs font-bold px-2.5 py-1 rounded-full ${addr.type==="Home"?"bg-blue-50 text-blue-600":addr.type==="Work"?"bg-purple-50 text-purple-600":"bg-gray-100 text-gray-600"}`}>
                              {addr.type==="Home"?"🏠":addr.type==="Work"?"💼":"📍"} {addr.type}
                            </span>
                            <div className="flex items-center gap-2 shrink-0">
                              {editingAddrIdx !== idx && (
                                <button onClick={() => handleEditAddress(idx)} className="text-xs font-semibold text-orange-500 border border-orange-200 px-3 py-1 rounded-lg hover:bg-orange-50 transition">✏️ Edit</button>
                              )}
                              <button onClick={() => handleDeleteAddress(idx)} className="text-xs font-semibold text-red-400 border border-red-200 px-3 py-1 rounded-lg hover:bg-red-50 transition">🗑️ Delete</button>
                            </div>
                          </div>
                          <p className="font-semibold text-gray-800 text-sm mt-2">{addr.name}</p>
                          <p className="text-gray-500 text-xs sm:text-sm mt-0.5 leading-relaxed">{addr.address}, {addr.city} – {addr.zipCode}</p>
